...
$jumlahData = count(query("SELECT * FROM transaksi_peminjaman"));
$jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
$halamanAktif = (isset($_GET["halaman"])) ? $_GET["halaman"] : 1;
$awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;
// $file = query("SELECT * FROM filem ORDER BY id DESC");
$file = query("SELECT * FROM transaksi_peminjaman ORDER BY id DESC LIMIT $awalData, $jumlahDataPerhalaman");
// tombol cari di klik
					...
					<?php foreach ($file as $row) : ?>
						<tr>
							...
							<td>
								...
								<td><?= date('Y-m-d', strtotime($row['tgl_pinjam'])); ?></td>
							</tr>
							<?php $i++; ?>
						<?php endforeach; ?>
					</tbody>
